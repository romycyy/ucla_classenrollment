...
def main():
    ...
    with sync_playwright() as p:
        ...
        if CHROME_DEBUGGING_PORT:
            # Connect to your already-running Chrome (real profile + cookies)
            try:
                ...
                print(
                    "Could not connect to Chrome. Start Chrome with (must use a non-default profile):\n"
                    '  "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" --user-data-dir="{}" --remote-debugging-port=9222\n'
                    "Then set CHROME_DEBUGGING_PORT=9222 in .env. Log in once in that window; cookies are saved there.\n"
                    "Error: {}".format(CHROME_DEBUG_PROFILE, e),
                    file=sys.stderr,
                )
        ...
        try:
            ...
            for name in ("Sign in", "Sign In", "登录", "Log in", "Login"):
                try:
                    page.get_by_role("link", name=name).first.click(timeout=1000)
                    signin_clicked = True
                    break
                except Exception:
                    try:
            ...
            page.wait_for_selector(
                "input[type='password']", state="visible", timeout=1000
            )
